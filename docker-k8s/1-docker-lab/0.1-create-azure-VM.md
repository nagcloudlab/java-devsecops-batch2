...
```bash
az vm open-port --resource-group myResourceGroup --name jenkinsVM --port '*'
```

get the public IP address of the VM

```bash
az vm list-ip-addresses --resource-group myResourceGroup --name jenkinsVM --output table
```
...
sudo ssh -i ~/.ssh/id_rsa azureuser@20.204.153.126
```
